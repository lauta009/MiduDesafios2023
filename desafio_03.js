...
const convertidorStringEnArrayPorSaltoDeLinea = (cadena) =>{
    return cadena.split(/\r\n|\r|\n/, -1)
}



const analizadorDeClaves = claves =>{
    let array = convertidorStringEnArrayPorSaltoDeLinea(claves)
    array.forEach(clave => {
        console.log('clave', clave)
        let separacion = clave.split(':')
        let password = separacion[1].replace(' ','');
        let limiteyLetra = separacion[0].split(' ');
        let letra = limiteyLetra[1];
        let minimo = limiteyLetra[0].split('-')[0]
        let maximo = limiteyLetra[0].split('-')[1]
        console.log('letra a evaluar', letra)
